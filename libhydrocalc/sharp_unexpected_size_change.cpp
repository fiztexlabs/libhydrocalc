...
void hydrocalc::SharpUnexpectedSizeChange::diagram41()
{
	...
	if (Re >= 3.3e3)
	{
		CSIlr_ = std::pow(1.0 - (A_  / A2_), 2.0);
	}
	if ((Re < 3.3e3) && (Re >= 500.0))
	{
		CSIlr_ = -8.44556 - 26.163 * std::pow(1.0 - (A_  / A2_), 2.0) - 5.38086 * std::pow(1.0 - (A_  / A2_), 4.0)
			+ std::log(Re) * (6.007 + 18.5372 * std::pow(1.0 - (A_  / A2_), 2.0) + 3.9978 * std::pow(1.0 - (A_  / A2_), 4.0))
			+ std::pow(std::log(Re), 2.0) * (-1.02318 - 3.0916 * std::pow(1.0 - (A_  / A2_), 2.0) - 0.680943 * std::pow(1.0 - (A_  / A2_), 4.0));
	}
	if ((Re < 500.0) && (Re >= 10.0))
	{
		CSIlr_ = 3.62536 + 10.744 * std::pow(1.0 - (A_  / A2_), 2.0) - 4.41041 * std::pow(1.0 - (A_  / A2_), 4.0)
			+ (1.0 / std::log(Re)) * (-18.13 - 56.77855 * std::pow(1.0 - (A_  / A2_), 2.0) + 33.40344 * std::pow(1.0 - (A_  / A2_), 4.0))
			+ (1.0 / std::pow(std::log(Re), 2.0)) * (30.8558 + 99.9542 * std::pow(1.0 - (A_  / A2_), 2.0) - 62.78 * std::pow(1.0 - (A_  / A2_), 4.0))
			+ (1.0 / std::pow(std::log(Re), 3.0)) * (-13.217 - 53.9555 * std::pow(1.0 - (A_  / A2_), 2.0) - 33.8053 * std::pow(1.0 - (A_  / A2_), 4.0));
	}
	if (Re < 10.0)
	{
		CSIlr_ = 30.0 / Re;
