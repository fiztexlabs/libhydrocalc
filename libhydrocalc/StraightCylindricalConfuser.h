...
private:
	...
	StraightCylindricalDiffuser* InvertConfuser;
	...
	*/
	StraightCylindricalDiffuser* createDiffuser();

protected:
	/**
	* @brief Check straight cylindrical confuser element values and raise exeptions or warnings.
	* @details Error codes: -
	*/
	void errorHandling();
...
public:
	...
	*/
		...
		: CylindricalHydraulicResistance()
	{
		...
		name_ = "StraightCylindricalConfuser" + std::to_string(id_);
	}
	...
	* @see Service
	*/
	StraightCylindricalConfuser(const std::string& name, const HSReal Re, const std::vector<HSReal>& G, bool invertable = true)
		: CylindricalHydraulicResistance(name, Re, 0.0, 0.0, 0.0, 0.0)
	{
		...
		FrictionPart = CylindricalFriction(name_ + "_friction", Re_, { rou_,D0_,L0_ });
