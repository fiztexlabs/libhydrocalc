...
private:
	...
	CurveCylindricalConfuser* InvertDiffuser;
	...
	* @return Pointer to CurveCylindricalConfuser object
	*/
	CurveCylindricalConfuser* createConfuser();

protected:
	/**
	* @brief Check curve cylindrical diffusers element values and raise exeptions or warnings.
	* @details Error codes: -
	*/
	void errorHandling();
...
public:
	...
	* @see Service
	*/
	CurveCylindricalDiffuser()
		: CylindricalHydraulicResistance()
	{
		// set default name of element
		name_ = "CurveCylindricalDiffuser" + std::to_string(id_);
	}
	...
	* @see Service
	*/
	CurveCylindricalDiffuser(const std::string& name, const HSReal Re, const std::vector<HSReal>& G, bool invertable = true)
